...
**Known limitations when importing emails with content block**: When importing emails with the “Import content blocks used in the selected emails” option checked, each email re-imports the associated content block/s, overriding any existing version. This results in the loss of any modifications made to the imported content blocks in Real-Time Marketing (RTM). Recommendation: To avoid unintended overwrites, import the content block first along with all related emails before making any modifications. Additionally, the content block dependency from Outbound Marketing (OBM) is stored in the msdynmkt_obmmigrationinfo column within the msdynmkt_fragment table. Clearing this value can prevent automatic overwriting during imports.
